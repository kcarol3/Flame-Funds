...
<!--    addCurrentAmount(id, amountToAdd) {-->
<!--      const token = sessionStorage.getItem("token");-->
<!--      const config = {-->
<!--        headers: { Authorization: `Bearer ${token}` },-->
<!--      };-->
...
<!--          // Znajdź indeks transakcji o podanym ID w tablicy transakcji-->
<!--          const indexToUpdate = transactionsForDate.findIndex((transaction) => transaction.id === id);-->

<!--          if (indexToUpdate !== -1) {-->
<!--            // Jeśli znaleziono indeks, updatuj kwotę transakcji-->
<!--            const updatedTransaction = transactionsForDate[indexToUpdate];-->
<!--            updatedTransaction.amount += amountToAdd; // Zaktualizuj kwotę transakcji o wartość "amountToAdd".-->

<!--            // Wyślij żądanie aktualizacji na serwer-->
<!--            axios-->
<!--                .put(`http://localhost:8741/api/financialGoal/addCurrentAmount/${id}`, { amount: amountToAdd }, config)-->
<!--                .then((response) => {-->
<!--                  if (response.status === 200) {-->
<!--                    console.log("Sukces: Aktualna kwota została zaktualizowana.");-->
<!--                  } else {-->
<!--                    console.error("Błąd: Aktualizacja kwoty nie powiodła się.");-->
<!--                  }-->
<!--                })-->
<!--                .catch((error) => {-->
<!--                  console.error("Błąd podczas edycji celu finansowego:", error);-->
<!--                });-->
<!--          }-->
<!--        }-->
<!--      }-->
